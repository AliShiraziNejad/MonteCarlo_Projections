...
def run_simulation(symbol, length, steps, centered):
    mc_norm = mcp_tda.Normal_MC(symbol, 'daily', length=length, daysBack=0)
    # mc_norm = mcp_yf.Normal_MC('^SPX', '1d', length=length, daysBack=0)  # for mcp_yf
...
if __name__ == "__main__":
    ...
    for symbol in symbols:
        for length, centered in configurations:
            run_simulation(symbol, length, steps, centered)
